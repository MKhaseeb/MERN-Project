...
export default function AllJobLists() {
  ...
  const [filters, setFilters] = useState(
    Object.fromEntries(Object.keys(FILTER_OPTIONS).map((key) => [key, ""]))
  );
  ...
  const toggleDropdown = (key) => {
    setDropdownStates((prev) => {
      ...
      if (isOpen && dropdownRefs.current[key]) {
        const rect = dropdownRefs.current[key].getBoundingClientRect();
        setDropdownPosition({
          x: rect.left,
      ...
      return Object.fromEntries(
        Object.keys(FILTER_OPTIONS).map((k) => [k, k === key ? isOpen : false])
      );
    });
  };
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    return (
      ...
      (!filters.Pay || salaryNum >= parseInt(filters.Pay, 10)) &&
      (!filters.Remote ||
        (filters.Remote === "Yes"
          ? location.includes("remote")
          : !location.includes("remote"))) &&
      (!filters.Company || company.includes(filters.Company.toLowerCase())) &&
      (!filters["Job Type"] || job.type === filters["Job Type"]) &&
      (!filters["Experience level"] ||
        job.experience === filters["Experience level"]) &&
  ...
  return (
    <div className="min-h-screen bg-[#0f1214] text-white">
      ...
      <div className="bg-[#0f1214] sticky top-0 z-30 border-b border-gray-800 px-6 py-4">
        <div className="max-w-7xl mx-auto overflow-x-auto relative">
            ...
            {Object.entries(FILTER_OPTIONS).map(([label, options]) => (
              <div key={label} className="relative inline-block text-left">
                ...
                {dropdownStates[label] && (
                  <DropdownPortal>
                    <div
                      className="absolute w-48 max-h-60 overflow-y-auto bg-[#1e2328] border border-gray-700 rounded shadow-xl z-[9999]"
                      style={{
                    ...
                    >
                      {options.map((option) => (
                        <div
                          ...
                          className="px-4 py-2 hover:bg-[#2c343c] cursor-pointer text-sm text-white"
                        >
                          {label === "Pay" ? `$${option}+` : option}
                        </div>
                      ))}
                    </div>
                  </DropdownPortal>
      ...
      <div className="flex px-6 py-8 gap-6 max-w-7xl mx-auto">
        <div className="w-2/5 pr-2 space-y-4">
          {filteredJobs.map((job) => (
            <div
              ...
              className={`cursor-pointer bg-[#161a1d] p-5 rounded-lg border ${
        ...
        <div
          ref={detailRef}
          className="w-3/5 bg-[#161a1d] p-8 rounded-lg border border-gray-800 shadow-xl sticky top-8"
        >
          {selectedJob ? (
